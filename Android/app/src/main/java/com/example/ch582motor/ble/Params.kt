...
object Params {
    ...
    const val POT_RAW_MAX = 12

    const val COUNT = 13

    /** Полная шкала АЦП, 12 бит. Ручка до неё физически не достаёт. */
    const val ADC_FULL_SCALE = 4095
    ...
    val all: List<ParamSpec> = listOf(
        ...
        ParamSpec(
            id = POT_RAW_MAX, key = "POT_RAW_MAX", title = "Верх хода ручки",
            unit = "", min = 100, max = ADC_FULL_SCALE, default = ADC_FULL_SCALE,
            kind = ParamKind.NUMBER,
            hint = "Отсчёт АЦП, который считается полным ходом. Ручка до 4095 " +
                "не достаёт: её верх сидит на делителе, иначе часть хода " +
                "упиралась бы в потолок АЦП. Выкрутите ручку в максимум, " +
                "посмотрите «Ручка» на экране наблюдения и впишите это число — " +
                "тогда упор даст ровно максимальную скважность.",
        ),
    )
