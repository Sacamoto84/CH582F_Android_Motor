 ...
 * @fn      pot_to_permille
 *
 * @brief   Отображение потенциометра в скважность между pwm_min и pwm_max.
 *
 *          Делим не на 4095, а на pot_raw_max: движок физически не достаёт
 *          до полной шкалы АЦП. Верх потенциометра сидит не на 3.3 В, а на
 *          делителе — иначе верхняя часть хода упиралась бы в потолок АЦП
 *          (полная шкала 2×VREF ≈ 2.1 В). Сколько именно даёт ручка на
 *          упоре, зависит от номиналов и разброса VREF, поэтому число
 *          снимается с конкретной платы и ставится по BLE.
 *
 *          Без этого ручка на упоре давала бы не pwm_max, а процентов 90-96
 *          от диапазона.
 */
static uint16_t pot_to_permille(void)
{
    uint32_t span = (uint32_t)(g_set.pwm_max - g_set.pwm_min);
    uint32_t raw  = s_pot_raw;
    uint32_t top  = g_set.pot_raw_max ? g_set.pot_raw_max : 4095U;

    if(raw > top) raw = top;     /* выше упора скважность не растёт */

    return (uint16_t)(g_set.pwm_min + (raw * span) / top);
}
 ...
 * @fn      boost_permille
 *
 * @brief   Скважность стартового рывка. boost_power — ПРИБАВКА к рабочей
 *          скважности, а не абсолютное значение.
 *
 *          Так рывок соразмерен режиму: на малой мощности он добавляет
 *          столько же, сколько на большой, и ручка сохраняет смысл.
 *          Абсолютное значение вело себя иначе — на малой мощности давало
 *          огромный скачок, на большой могло оказаться слабее рабочей
 *          скважности вовсе.
 *
 *          Сумма зажимается по 1000: выше полной скважности не бывает,
 *          и на верху хода ручки прибавка просто упирается в потолок.
 */
static uint16_t boost_permille(void)
{
    uint32_t b = (uint32_t)pot_to_permille() + g_set.boost_power;

    return (uint16_t)(b > 1000U ? 1000U : b);
}
 ...
 * @fn      Motor_WokeByKey
 *
 * @brief   Вышли из глубокого сна. Источник пробуждения там ровно один —
 *          кнопка PB22, см. Board_Sleep().
 *
 *          Глубокий сон это LowPower_Shutdown, то есть полный сброс: чип
 *          стартует с main() и о разбудившем нажатии ничего не знает.
 *          Кнопка отработала как источник пробуждения, а автомат кнопки
 *          инициализируется с нуля уже после. Отпустили раньше, чем дошло
 *          до опроса, — события нажатия просто не возникает, и помпа
 *          не запускается. Поэтому запускаем её здесь: пользователь нажал,
 *          чтобы налить воды, а не чтобы разбудить устройство.
 *
 *          Второе обязательно: то же нажатие надо проглотить. Иначе, если
 *          кнопку ещё держат, автомат досчитает дебаунс, выдаст своё
 *          нажатие, Motor_Toggle() увидит уже работающую помпу — и остановит
 *          её через пару десятков миллисекунд после пуска.
 *
 *          Щелчок озвучивается здесь же. В poll_key() он звучит на нажатии,
 *          но сюда автомат кнопки не доходит — нажатие уже проглочено выше.
 *          Порядок тот же, что в poll_key(): сначала щелчок, потом пуск,
 *          так что при отказе по разряду buzzer_error() оборвёт его и
 *          прозвучит вместо него.
 */
void Motor_WokeByKey(void)
{
    UB_AssumePressed(&s_btn);
    buzzer_ios_click();
    (void)Motor_Start();
}
...
void Motor_Init(void)
{
    ...
    /* Входной буфер включён — источники высокоомные (потенциометр с верхним
     * резистором, делитель 22к/18к). PGA 0 дБ: полная шкала 2*VREF = 2.1 В,
     * оба канала спроектированы под этот потолок. */
